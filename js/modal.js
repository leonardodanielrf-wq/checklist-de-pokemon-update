...
export async function openPokemon(id) {
  if (id === currentPokemonId) return; // evita reload innecesario
  currentPokemonId = id;
  ...
  // LIMPIAR CONTENIDO
  document.getElementById("pokeName").textContent = "Cargando...";
  ...
  try {
    // FETCH POKEMON (cache si existe)
    const p = pokemonCache[id] || await fetchPokemon(id);
    pokemonCache[id] = p;

    if (id !== currentPokemonId) return; // check race condition

    renderPokemon(p, canvas);

    // FETCH EVOLUCIONES (cache si existe)
...
function renderPokemon(p, canvas) {
  // NOMBRE
  document.getElementById("pokeName").textContent = "#" + p.id + " " + p.name;
  ...
  const types = p.types.map(t => tiposES[t.type.name] || t.type.name).join(", ");
  document.getElementById("pokeTypes").innerHTML = "<b>Tipo:</b> " + types;
  ...
  let statsHTML = "";
  p.stats.forEach(s => {
    let nombre = statsES[s.stat.name] || s.stat.name;
    statsHTML += `<div class="stat"><span>${nombre}</span><span>${s.base_stat}</span></div>`;
  });
  document.getElementById("pokeStats").innerHTML = "<h3>Stats</h3>" + statsHTML;
